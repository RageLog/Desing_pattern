...
Flyweight::Flyweight(const Shared_t& key) 
:m_key(std::make_shared<Shared_t>(key)){}
Flyweight::~Flyweight(){}
void Flyweight::doSomeThing() const
{ 
    std::cout << "Flyweight doing SomeThing\n";
    std::cout << "and its member is:\n";
    std::cout << m_key->intData << " (int member) "<< " and " << m_key->strData << " (string member) \n";
}
...
Flyweight& flyweightFactory::getFlyweight(const Shared_t& key) 
{
    ...
    {
        std::cout << "Key is found " << key.intData << " (int member) "<< " and " <<key.strData << " (string member) \n\n";
        return *m_Flyweights[key.id];
    }
    m_Flyweights[key.id] = std::make_shared<Flyweight>(key);
    std::cout << "New Flyweigts generated with: " << key.intData << " (int member) "<< " and " <<key.strData << " (string member) \n\n";
    return *m_Flyweights[key.id];
}
...
void flyProduct::doSomeThing() const
{
    ...
    std::cout << someUniqueInt << " (int member) "<< " and " << someUniqueString << " (string member) \n\n";
}